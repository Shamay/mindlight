export const compareAverages = (previousAverageData) => (
  currentAverageData
) => {
  let percentChange = 0;
  const interval = 10;
  ...
  if (currentAverageData.count % interval === 0) {
    ...
    if (previousAverageData) {
      percentChange =
        (currentAverageData.avg / previousAverageData.avg - 1) * 100;
...
export const averageProbability = (runningAverage) => (probability) => {
  ...
  const newAverage =
    (runningAverage.avg * runningAverage.count + probability) / newCount;
